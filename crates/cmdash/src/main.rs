...
//! with the binary owning one command-induced graph of panes per
//! tab. v1 is single-tab, sync IO via per-pane reader threads,
//! no input loop, no dashcompositor wiring — degraded text-mode
//! per AGENTS.md §"Rendering pipeline" step 7.
...
fn run() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let result = tick_loop(&mut runners, guard.as_mut(), tick);
    drop(guard);
    result
}
...
/// error or normal return. Without this guard, an early `?` in the
/// setup between `enable_raw_mode()` and `tick_loop()` would
/// strand the user in the alternate screen.
///
